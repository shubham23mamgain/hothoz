...
import { updateBasePizza } from "../../../features/actions/pizza/patchCustomization";

export default function EditBaseModal({ data, setModal }) {
  ...
  function onSubmit(formData) {
    ...
    const priceFilter = price.map((item) => {
      return { size: item?.size?.value, price: item?.price };
    });
    const newData = { name, price: priceFilter };

    dispatch(updateBasePizza({_id:data?._id,...newData}));
    setModal((prev) => !prev);
  }
